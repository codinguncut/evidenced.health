---
type: framework
question: How much dietary protein retains fat-free mass during energy restriction in a resistance-trained adult, and how does that differ from the energy-balance target?
aliases: [Protein for Fat-Free Mass Retention, Protein During Weight Loss, Protein FFM Deficit, Protein Cutting Diet, Protein Energy Deficit, FFM Retention Protein Target, Protein During Caloric Deficit]
authors: [Refalo, Martin C; Trexler, Eric T; Helms, Eric R]
sources: [Refalo - Protein Fat-Free Mass Energy Restriction 2025]
cluster: muscle
confidence: low
relationships:
  related_to:
...
- **Leaner -> higher target** (BF% inverse scaling); **the obese are off-support** — this evidence does
  not transport to them, and the per-FFM denominator only sharpens that the number is undefined until the
  kg is named.
- **On a GLP-1 drug:** this is the protein side of the muscle-preservation strategy — but appetite
  ...
  -> [[GLP-1 and Lean Mass]].
- **Open loop / low confidence:** exploratory meta-regression on a surrogate; nothing here grades a chosen
  protein intake against a realized function or health outcome. Treat as a well-reasoned prior for
  practice, revisable on controlled higher-vs-lower RCTs in this population.
