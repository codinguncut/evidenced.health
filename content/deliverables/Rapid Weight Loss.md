---
type: deliverable
title: Rapid Weight Loss
redirect: Body Fat
question: For an adult who has decided to lose weight, does the rate of loss change regain, muscle loss, and organ harm over and above the amount lost and the method used — and does a diet-created deficit differ from an exercise-widened one for body composition?
aliases: [Rapid Weight Loss, How Fast Is Too Fast, Rate of Weight Loss, Crash Diet, Fast vs Slow Weight Loss, Speed of Weight Loss]
created: 2026-07-30
updated: 2026-08-10
---

*This page has been folded into [[Body Fat]].*

The rate-of-loss question — *how fast is too fast?* — is now **section 7 ("How you take it
off: rate, composition, and the realistic alternative")** of [[Body Fat]]. The short answer
is unchanged: rate is a minor knob. What protects a person while fat comes off is preserving
lean mass (adequate protein plus resistance training) and choosing a method they can sustain;
much of what people fear about *speed* tracks the *method* (protein-poor, unsupervised
regimens), not the number on the scale. Supervised rapid loss — as in DiRECT's 825-853 kcal/day
total diet replacement — is safe and delivered the largest diabetes-remission signal in the
corpus. The rate-vs-regain and speed-vs-muscle magnitudes remain named gaps.

See [[Body Fat]] for the full treatment, including where the fat is stored, whether losing it
reaches hard outcomes, and how the answer varies by stratum.
