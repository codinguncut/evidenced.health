...
They define the reference class and are the first inputs to the ESC SCORE2
...
The single largest modifiable exposure, and free to ask. Current smokers die at
...
### Blood pressure (with resting heart rate)

Blood pressure is a SCORE2 input, and it flags one of the
...
Places a person on the dysglycemia / metabolic-syndrome axis — the stratum where weight
...
The cheapest routine screen for the fatty-liver stratum, which this metabolic
...
### Lp(a), measured once in a lifetime

Lipoprotein(a) is a genetic, apoB-containing atherogenic
...
### apoB, where available — the upgrade over non-HDL

apoB is the direct particle count and the most
...
### Grip strength — high-yield if available, but flagged

Low grip strength is among the strongest
...
Waist is a cheaper, population-available proxy for the
...
This is the clearest skip, and the wiki's own worked case shows why.
...
Largely overlaps HbA1c, which needs no fast and is less noisy. Capturing both
adds little; prefer HbA1c.

### LDL-C as the *headline* lipid

Not dropped — keep it in the panel — but for the metabolically-
...
### Capture, not interpret or prescribe

Reference ranges, diagnosis and acting on any of these
...
This list grades what the held evidence says *changes a decision*. It does not,
...
### A general default, tuned per person

Which metrics matter most depends on the individual: a lean,
