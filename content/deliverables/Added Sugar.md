...
- **The loop is open.** This appraises the published evidence for coherence and fidelity to its sources;
  no step here tests a recommendation against a realised outcome in a person. A clean appraisal is not a
  validated result.
- **This appraises evidence; it does not prescribe.** Selecting, dosing and screening for individual
  circumstances (medications, history, dental and metabolic status) are clinical acts outside this scope.
- **Estimates are population-level by default.** The magnitudes above are averages over studied groups;
  absolute benefit scales with a person's baseline risk, and any individual sits somewhere the population
  estimate only approximates. Stratify per person.
- **One health axis only.** This weighs sugar on health outcomes. Cost, environmental load and other
  axes are real but are not priced here; where they trade against health, that weighing is the person's.
